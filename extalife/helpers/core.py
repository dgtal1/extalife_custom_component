...
from .const import DATA_CORE, DOMAIN, SIGNAL_CONN_STATE_CHANGE
...
class Core:
    ...
    def __init__(self, config_entry: ConfigEntry):
        ...
        self._is_disconnect_suspend = False

    @classmethod
    async def options_change_callback(cls, hass, config_entry: ConfigEntry):
        """Options update listener"""
        core = Core.get(config_entry.entry_id)

        if not core._is_disconnect_suspend:
    ...
    def _on_disconnect_callback(self):
        ...
        if self._is_disconnect_suspend:
            return  # no reconnection after on-demand disconnect
    ...
    async def disconnect_suspend(self):
        """Disconnect API from controller and stop polling = suspend integration"""
        self._is_disconnect_suspend = True

        await self.data_manager.async_stop_polling()
        await self.api.disconnect()

    async def connect_resume(self):
        """Connect API to controller and start/resume polling = resume integration"""

        await self.api.async_reconnect()
        await self.data_manager.async_start_polling(poll_now=True)
    ...
    def unregister_signal_callbacks(self):
        for callback in self.signal_remove_callbacks:
            callback()
            if callback:
                self.signal_remove_callbacks.remove(callback)
    ...
    def async_signal_send(self, signal: str, *args: Any, sync=False) -> None:
        """Send signal and data.

        This method must be run in the event loop.
        sync - when True the callback must be marked as @callback for it to work, otherwise non-thread safe operation error
        """
        ...
        for target in target_list:
            ...
            if sync:
                self._hass.add_job(target, *args)
            else:
