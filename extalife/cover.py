...
from .helpers.const import OPTIONS_COVER_INVERTED_CONTROL
...
class ExtaLifeCover(ExtaLifeChannel, CoverEntity):
    ...
    def device_class(self):
        ...
        if chn_type == GATE_CHN_TYPE_WICKET:
    ...
    def current_cover_position(self):
        ...
        if (
            self.is_exta_free
            or self.device_class == DEVICE_CLASS_GATE
            or self.device_class == DEVICE_CLASS_DOOR
        ):
            return
    ...
    async def async_open_cover(self, **kwargs):
        ...
        pos = (
            1
            if self.device_class == DEVICE_CLASS_GATE
            or self.device_class == DEVICE_CLASS_DOOR
            else ExtaLifeCover.POS_OPEN
        )  # ROB-21 to open 'pos' must be different from 0
        if not self.is_exta_free:
            action = (
                ExtaLifeAPI.ACTN_SET_POS
                if self.device_class != DEVICE_CLASS_GATE
                and self.device_class != DEVICE_CLASS_DOOR
                else ExtaLifeAPI.ACTN_SET_GATE_POS
            )
